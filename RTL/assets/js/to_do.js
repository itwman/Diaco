...
$(document).on("submit", "#add_employee_todo", function (e) {
  ...
  try {
    const newItem = {
      ...
      notes: `<span class="text-dark f-s-14"><i class="ti ti-circle-filled ms-2 f-s-6"></i>${notesField.val()}</span>`,
      date: dateField.val(),
      status: `<span class="badge text-uppercase f-s-10 bg-${statusField.val()}-subtle bg-${statusField.val()}">${$(
        "#status-field option:selected"
      ).text()}</span>`,
    };
...
$(document).on("click", "#edit-btn", function () {
  try {
    ...
    item.values({
      ...
      date: dateField.val(),
      status: `<span class="badge text-uppercase bg-${statusField.val()}-subtle bg-${statusField.val()}">${$(
        "#status-field option:selected"
      ).text()}</span>`,
    });
    clearFields();
